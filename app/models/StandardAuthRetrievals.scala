...
case class StandardAuthRetrievals(enrolments: Set[Enrolment],
  ...
  def userLink: String = {
    "/ated/" + atedReferenceNumber.getOrElse {
      Logger.warn(s"[AtedContext][userLink] Exception - User does not have the correct authorisation")
      throw new UnauthorizedException("User does not have the correct authorisation")
    }
  }
  ...
  def atedReferenceNumber: Option[String] = {
    val atedRefNumber = getEnrolment(atedKey)
    if(isAgent) {
      Logger.warn(s"AgentRefNumber:" + agentRefNo)
      Logger.warn(s"AtedUtr:" + atedRefNumber)
      None
    } else {
