...
import models.PropertyDetailsFirstOccupiedKnown
...
class PropertyDetailsFirstOccupiedController @Inject()(mcc: MessagesControllerComponents,
                                                  ...
                                                  template: views.html.propertyDetails.propertyDetailsFirstOccupied)
  ...
  val controllerId: String = "PropertyDetailsFirstOccupiedController"
            ...
            case PropertyDetailsCacheSuccessResponse(propertyDetails) =>
              currentBackLink.flatMap { backLink =>
                dataCacheConnector.fetchAndGetFormData[Boolean](SelectedPreviousReturn).flatMap { isPrevReturn =>
                  val displayData = PropertyDetailsFirstOccupiedKnown(propertyDetails.value.flatMap(_.isWhenFirstOccupiedKnown)
                  )
                  Future.successful(Ok(template(id,
                    propertyDetails.periodKey,
                    firstOccupiedKnownForm.fill(displayData),
  ...
  def save(id: String, periodKey: Int, mode: Option[String]): Action[AnyContent] = Action.async { implicit request =>
    authAction.authorisedAction { implicit authContext =>
      ensureClientContext {
        serviceInfoService.getPartial.flatMap { serviceInfoContent =>
          firstOccupiedKnownForm.bindFromRequest.fold(
            formWithError => {
