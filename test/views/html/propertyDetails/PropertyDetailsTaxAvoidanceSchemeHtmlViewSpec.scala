...
class PropertyDetailsTaxAvoidanceSchemeHtmlViewSpec extends AtedViewSpec with MockitoSugar with MockAuthUtil {
  ...
  private val form = PropertyDetailsForms.propertyDetailsTaxAvoidanceSchemeForm
    .withError("isTaxAvoidance", "ated.property-details-period.isTaxAvoidanceScheme.error-field-name")
  ...
  "The Property Details Professionally Valued View page" must {
    ...
    "have a the correct page title" in {
      doc.title mustBe TitleBuilder.buildErrorTitle(messages("ated.property-details-period.isTaxAvoidanceScheme.title"))
    }
    "have the correct page header" in {
      doc.getElementsByTag("h1").text() must include(messages("ated.property-details-period.isTaxAvoidanceScheme.header"))
    }
    ...
    "check page errors when no radio buttons has been selected" in {
      val eform = Form(form.mapping, form.data,
        Seq(FormError("isTaxAvoidance", messages("ated.property-details-period.isTaxAvoidanceScheme.error-field-name")))
        , form.value)
      ...
      errorDoc.getElementById("isTaxAvoidance-error").text() mustBe ("Error: " + messages("ated.property-details-period.isTaxAvoidanceScheme.error-field-name"))
      errorDoc.getElementsByClass("govuk-error-summary__list").text() mustBe (messages("ated.property-details-period.isTaxAvoidanceScheme.error-field-name"))
    }
