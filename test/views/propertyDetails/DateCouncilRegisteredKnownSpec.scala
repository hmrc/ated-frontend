...
import forms.PropertyDetailsForms.dateCouncilRegisteredKnownForm

class DateCouncilRegisteredKnownSpec extends AnyFeatureSpec with GuiceOneAppPerSuite with MockitoSugar
  ...
  val title: String = "Do you know when the local council registered the property for council tax?"
  ...
  Feature("The user views the Do you know when the local council registered the property for council tax? page") {

    info("as a client I want to view the Do you know when the local council registered the property for council tax? page")

    Scenario("describe the page") {

      Given("the client has not entered any information")
      When("The client views the page")
      val html = injectedViewInstance("1", dateCouncilRegisteredKnownForm, None, Html(""), Some("http://backLink"))
    ...
    Scenario("describe the page when editing a return") {
      ...
      Given("the client has not entered any information")
      When("The client views the page")
      val html = injectedViewInstance("1", dateCouncilRegisteredKnownForm, Some(utils.AtedUtils.EDIT_SUBMITTED), Html(""), Some("http://backLink"))
      val document = Jsoup.parse(html.toString())
