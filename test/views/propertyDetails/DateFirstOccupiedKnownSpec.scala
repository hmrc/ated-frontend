...
import forms.PropertyDetailsForms.firstOccupiedKnownForm
...
class DateFirstOccupiedKnownSpec extends AnyFeatureSpec with GuiceOneAppPerSuite with MockitoSugar
  ...
  Feature("The user views the Do you know when the property was first occupied page") {
    ...
    Scenario("describe the page") {
      ...
      val html = injectedViewInstance("1", firstOccupiedKnownForm, None, Html(""), Some("http://backLink"))
      val document = Jsoup.parse(html.toString())
      Then("The title should match - Do you know when the property was first occupied? - GOV.UK")
      assert(document.title() === "Do you know when the property was first occupied? - GOV.UK")

      Then("The header should match - Do you know when the property was first occupied?")
      assert(document.getElementsByTag("h1").text() contains "Do you know when the property was first occupied?")
    ...
    Scenario("describe the page when editing a return") {
      ...
      Given("the client has not entered any information")
      When("The client views the page")
      val html = injectedViewInstance("1", firstOccupiedKnownForm, Some(utils.AtedUtils.EDIT_SUBMITTED), Html(""), Some("http://backLink"))
      val document = Jsoup.parse(html.toString())
      Then("The title should match - Do you know when the property was first occupied? - GOV.UK")
      assert(document.title() === "Do you know when the property was first occupied? - GOV.UK")
