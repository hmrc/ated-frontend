...
class editLiabilitySummarySpec extends FeatureSpec with OneServerPerSuite with MockitoSugar with BeforeAndAfterEach with GivenWhenThen{
  ...
  feature("The user can view their property details summary before they submit it") {
    ...
    scenario("Amended charge with periods") {
      ...
      val html = views.html.editLiability.editLiabilitySummary(propertyDetails, "A", displayPeriods, PeriodUtils.getCalculatedPeriodValues(propertyDetails.calculated), None)
      ...
      assert(document.getElementById("ated-charge-text-amended").text() === "Based on the information you have given us your revised ATED charge for this amended return is")
    ...
    scenario("Changed charge with periods") {
      ...
      val displayPeriods = PeriodUtils.getDisplayPeriods(propertyDetails.period)
      assert(displayPeriods.size === 2)
      val html = views.html.editLiability.editLiabilitySummary(propertyDetails, "C", displayPeriods, PeriodUtils.getCalculatedPeriodValues(propertyDetails.calculated), Some("http://backLink"))
      ...
      assert(document.getElementById("ated-charge-text-changed").text() === "Based on the information you have given us your revised ATED charge for this changed return is")
      assert(document.getElementById("ated-charge-value").text() === "£1,000")
    ...
    scenario("Futher Charge the basic summary with no periods") {
      ...
      val html = views.html.editLiability.editLiabilitySummary(propertyDetails, "F", Nil, PeriodUtils.getCalculatedPeriodValues(propertyDetails.calculated), Some("http://backLink"))
      ...
      assert(document.getElementById("return-status-header").text() === "Return status")
      assert(document.getElementById("return-status-label").text() === "Status")
      assert(document.getElementById("ated-charge-text-further").text() === "Based on the information you have given us your revised ATED charge for this further return is")
      assert(document.getElementById("ated-charge-text-amended") === null)
      assert(document.getElementById("ated-charge-text-changed") === null)
      assert(document.getElementById("ated-charge-value").text() === "£1,000")
