  ...
  "BackLinkCacheConnector" must {
    ...
    "saveAndReturnBusinessDetails" must {
      ...
      "save the back link" in {
        ...
        when(mockSessionCache.cache[BackLinkModel](Matchers.any(), Matchers.any())(Matchers.any(), Matchers.any())).thenReturn(Future.successful(returnedCacheMap))
    ...
    "clearBackLinks" must {
      ...
      "clear the back link details when we have none" in {

        implicit val hc: HeaderCarrier = HeaderCarrier()
        val result = TestDataCacheConnector.clearBackLinks(Nil)
