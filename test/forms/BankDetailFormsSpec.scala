...
import forms.BankDetailForms.{bankDetailsForm, hasUkBankDetailsForm}
import models.{BicSwiftCode, HasUkBankDetails, Iban, SortCode}
...
class BankDetailFormsSpec extends PlaySpec with GuiceOneServerPerSuite {
  ...
  "bankDetailsForm" must {
    ...
    "bind successfully when 'hasUkBankDetails' is true" in {
      val data: Map[String, String] = Map("hasUkBankDetails" -> "true")
      val boundForm: Form[HasUkBankDetails] = hasUkBankDetailsForm.bind(data)
      boundForm.hasErrors mustBe false
      boundForm.value mustBe Some(HasUkBankDetails(Some(true)))
    }

    "show an error when 'hasUkBankDetails' is missing" in {
      val data: Map[String, String] = Map.empty
      val boundForm: Form[HasUkBankDetails] = hasUkBankDetailsForm.bind(data)
      boundForm.hasErrors mustBe true
      boundForm.errors.size mustBe 1
      boundForm.error("hasUkBankDetails").map(_.message) mustBe Some("ated.bank-details.error-key.hasUkBankDetails.empty")
    }
