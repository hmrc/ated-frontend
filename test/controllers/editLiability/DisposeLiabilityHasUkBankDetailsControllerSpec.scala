...
import models.{BankDetailsModel, DisposeLiabilityReturn, HasUkBankDetails}
...
class DisposeLiabilityHasUkBankDetailsControllerSpec
  ...
  private val injectedView = app.injector.instanceOf[views.html.editLiability.disposeLiabilityHasUkBankDetails]
  ...
  class Setup {
    ...
    val controller = new DisposeLiabilityHasUkBankDetailsController(
      mockMcc,
  ...
  "DisposeLiabilityHasBankDetailsController" must {
    ...
    "save" should {
      ...
      "return BAD_REQUEST for invalid data" in new Setup {
        val invalidJson: JsValue = Json.parse("""{"hasUkBankDetails": "0"}""")
      ...
      "redirect to UK bank details page if user has a UK bank account" in new Setup {
        val inputJson: JsValue = Json.toJson(HasUkBankDetails(Some(true)))
      ...
      "redirect to non-UK bank details page if user does not have a UK bank account" in new Setup {
        val inputJson: JsValue = Json.toJson(HasUkBankDetails(Some(false)))
        val returnData: Some[DisposeLiabilityReturn] = Some(DisposeLiabilityReturnBuilder.generateDisposeLiabilityReturn("123456789012"))
        when(mockBackLinkCacheConnector.saveBackLink(any(), any())(any()))
          .thenReturn(Future.successful(None))
      ...
      "redirect to Account Summary page if dispose liability is not returned for a user with UK bank account" in new Setup {
        val inputJson: JsValue = Json.toJson(HasUkBankDetails(Some(true)))
        when(mockBackLinkCacheConnector.saveBackLink(any(), any())(any()))
          .thenReturn(Future.successful(None))

        saveWithAuthorisedUser(inputJson) { result =>
          status(result) mustBe SEE_OTHER
          redirectLocation(result) mustBe Some("/ated/account-summary")
        }
      }
