...
class EditLiabilityDeclarationControllerSpec extends PlaySpec with OneServerPerSuite with MockitoSugar with BeforeAndAfterEach {
  ...
  def viewWithAuthorisedUser(x: Option[PropertyDetails] = None)(test: Future[Result] => Any) {
    ...
    when(mockChangeLiabilityReturnService.retrieveSubmittedLiabilityReturnAndCache(Matchers.eq(formBundleNo1), Matchers.any())(Matchers.any(), Matchers.any())).thenReturn(Future.successful(x))
  ...
  def submitWithAuthorisedUser(a: Option[PropertyDetails], oldForBundle: String = formBundleNo1)(test: Future[Result] => Any) {
    ...
    when(mockDataCacheConnector.fetchAtedRefData[String](Matchers.eq(AtedConstants.DelegatedClientAtedRefNumber))
      (Matchers.any(), Matchers.any(), Matchers.any())).thenReturn(Future.successful(Some("XN1200000100001")))
    when(mockChangeLiabilityReturnService.retrieveSubmittedLiabilityReturnAndCache(Matchers.eq(formBundleNo1), Matchers.any())(Matchers.any(), Matchers.any())).thenReturn(Future.successful(a))
