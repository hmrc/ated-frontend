...
import views.html.editLiability.editLiabilitySent
...
class EditLiabilitySentControllerSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar with BeforeAndAfterEach with MockAuthUtil {
  ...
  val injectedViewInstance: editLiabilitySent = app.injector.instanceOf[views.html.editLiability.editLiabilitySent]
...
class Setup {
  ...
  def viewWithAuthorisedUser(x: Option[EditLiabilityReturnsResponseModel] = None)(test: Future[Result] => Any): Unit = {
  ...
  def getPrintFriendlyWithAuthorisedUser(x: Option[EditLiabilityReturnsResponseModel] = None)(test: Future[Result] => Any): Unit = {
    val userId = s"user-${UUID.randomUUID}"
    val authMock = authResultDefault(AffinityGroup.Organisation, defaultEnrolmentSet)
    setAuthMocks(authMock)
    when(mockDataCacheService.fetchAndGetData[String](ArgumentMatchers.eq(AtedConstants.DelegatedClientAtedRefNumber))
      ...
      "return amended return sent page, if response found in cache and amountDueOrRefund is Negative" in new Setup {
        ...
        viewWithAuthorisedUser(Some(resp)) {
          result =>
            ...
            document.getElementsByClass("govuk-panel__title").text() must be("Your amended return has been successfully submitted")
            ...
            document.getElementById("late-payment")
              .text() must be("Late payment penalties can be issued when ATED is unpaid. Find out about how to pay and payment deadlines (opens in new tab).")
      ...
      "return further return sent page, if response found in cache and amountDueOrRefund is Negative" in new Setup {
        ...
        viewWithAuthorisedUser(Some(resp)) {
          result =>
            ...
            document.getElementsByClass("govuk-panel__title").text() must be("Your further return has been successfully submitted")
            document.getElementById("view-message")
              .text() must be("You can view your completed returns, overall balance, payment references and ways to pay in the ATED online service.")
            document.getElementById("email-message").text() must be("You will not receive an email confirmation.")
            document.getElementById("charges-heading").text() must be("Charges for this return")
      ...
      "return edit details return sent page, if response found in cache and amountDueOrRefund is Negative" in new Setup {
        ...
        viewWithAuthorisedUser(Some(resp)) {
          result =>
            ...
            document.getElementsByClass("govuk-panel__title").text() must be("Your change in details has been successfully submitted")
            document.getElementById("view-message")
              .text() must be("You can view your completed returns, overall balance, payment references and ways to pay in the ATED online service.")
            document.getElementById("email-message").text() must be("You will not receive an email confirmation.")
            document.getElementById("charges-heading").text() must be("Charges for this return")
