...
class DisposeLiabilityNonUkBankDetailsControllerSpec
  ...
  private val injectedView = app.injector.instanceOf[views.html.editLiability.disposeLiabilityNonUkBankDetails]
  ...
  class Setup {
    ...
    val controller = new DisposeLiabilityNonUkBankDetailsController(
      mockMcc,
    ...
    def saveWithAuthorisedFormUser(fakeRequest: FakeRequest[AnyContentAsFormUrlEncoded], disposeReturn: Option[DisposeLiabilityReturn] = None)
      ...
      when(mockDataCacheService.fetchAndGetData[String](ArgumentMatchers.eq(AtedConstants.DelegatedClientAtedRefNumber))(using any(), any()))
        .thenReturn(Future.successful(Some("XN1200000100001")))
      when(mockDisposeLiabilityReturnService.cacheDisposeLiabilityReturnBank(any(), any())(using any(), any()))
        .thenReturn(Future.successful(disposeReturn))
  ...
  "DisposeLiabilityNonUkBankDetailsController"  must {
    "view" should {
      ...
      "return OK when liability return is found" in new Setup {
        ...
        viewWithAuthorisedUser(Some(returnData)) { result =>
          ...
          doc.title() must include("Enter your bank account details")
        }
      }
    ...
    "save" should {
      ...
      "redirect to summary page on successful non-UK bank form submission" in new Setup {
