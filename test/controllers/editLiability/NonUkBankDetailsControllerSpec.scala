...
class NonUkBankDetailsControllerSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar with BeforeAndAfterEach with MockAuthUtil {
  ...
  class Setup {
    ...
    val testBankDetailsController: NonUkBankDetailsController = new NonUkBankDetailsController(
      mockMcc,
    ...
    def viewWithAuthorisedUser(changeLiabilityReturnOpt: Option[PropertyDetails])(test: Future[Result] => Any): Unit = {
      ...
      when(mockBackLinkCache.fetchAndGetBackLink(ArgumentMatchers.any())(using ArgumentMatchers.any())).thenReturn(Future.successful(Some("http://backlink")))
  ...
  "BankController" must {
    ...
    "save - for authorised user" must {
      ...
      "for a valid non-UK bank account details, redirect to the summary page" in new Setup {
