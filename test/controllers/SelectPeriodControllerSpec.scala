...
class SelectPeriodControllerSpec extends PlaySpec with GuiceOneAppPerSuite with MockitoSugar with MockAuthUtil {
  ...
  class Setup(endYear : Int = baseYear) {
    ...
    def getWithAuthorisedUser(test: Future[Result] => Any): Unit = {
      ...
      when(mockDataCacheService.fetchAndGetData[String](ArgumentMatchers.any())
    ...
    def getWithAuthorisedUserWithSavedData(test: Future[Result] => Any): Unit = {
      ...
      when(mockDataCacheService.fetchAndGetData[SelectPeriod](ArgumentMatchers.any())
        (using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(Some(SelectPeriod(Some("2015")))))
          ...
          "with invalid form, return BadRequest" in new Setup {
            ...
            submitWithAuthorisedFormUser(FakeRequest()
              ...
              Some("XN1200000100001")
                ...
                doc.getElementsByAttributeValue("for", "period-8").text() must be(s"2018 to 2019")
                doc.getElementsByAttributeValue("for", "period-7").text() must be(s"2019 to 2020")
                doc.getElementsByAttributeValue("for", "period-6").text() must be(s"2020 to 2021")
