...
class DateFirstOccupiedControllerSpec extends PlaySpec with GuiceOneServerPerSuite with BeforeAndAfterEach with MockitoSugar with MockAuthUtil {
  ...
  class Setup {
    ...
    def getWithAuthorisedUser(test: Future[Result] => Any): Any = {
      ...
      when(mockDataCacheService.fetchAndGetData[DateFirstOccupied](ArgumentMatchers.eq(AtedConstants.NewBuildFirstOccupiedDateKnown))
        (using ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(Some(DateFirstOccupied(None))))
