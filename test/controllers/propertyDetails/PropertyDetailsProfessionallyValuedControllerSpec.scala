...
class PropertyDetailsProfessionallyValuedControllerSpec
    extends PlaySpec
  ...
  class Setup {
    ...
    val testPropertyDetailsProfessionallyValuedController: PropertyDetailsProfessionallyValuedController =
      new PropertyDetailsProfessionallyValuedController(
        mockMcc,
        mockAuthAction,
    ...
    def editDataWithAuthorisedUser(id: String, propertyDetails: PropertyDetails)(test: Future[Result] => Any): Unit = {
      val userId   = s"user-${UUID.randomUUID}"
      val authMock = authResultDefault(AffinityGroup.Organisation, defaultEnrolmentSet)
      setAuthMocks(authMock)
      when(mockDataCacheService.fetchAndGetData[Boolean](ArgumentMatchers.any())(using ArgumentMatchers.any(), ArgumentMatchers.any()))
    ...
    def submitWithAuthorisedUser(fakeRequest: FakeRequest[AnyContentAsFormUrlEncoded])(test: Future[Result] => Any): Unit = {
      ...
      when(
        ...
        .thenReturn(Future.successful(Some("XN1200000100001")))
      when(
        mockPropertyDetailsService.saveDraftPropertyDetailsProfessionallyValued(ArgumentMatchers.any(), ArgumentMatchers.any())(using
