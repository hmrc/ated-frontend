...
class PropertyDetailsProfessionallyValuedControllerSpec extends PlaySpec with OneServerPerSuite with MockitoSugar with BeforeAndAfterEach {
  ...
  object TestPropertyDetailsController extends PropertyDetailsProfessionallyValuedController {
  ...
  "PropertyDetailsProfessionallyValuedController" must {
    ...
    "propertyDetails" must {
      ...
      "not respond with NOT_FOUND when we try to view an id" in {
        val result = route(FakeRequest(GET, "/ated/liability/create/valued/view/1"))
      ...
      "Authorised users" must {
        ...
        "show the chargeable property details edit view if we come from the summary page and have data" in {
          val propertyDetails = PropertyDetailsBuilder.getPropertyDetails("1", Some("postCode")).copy(value = None)
  ...
  def submitWithAuthorisedUser(inputJson: JsValue)(test: Future[Result] => Any) {
    ...
    when(mockDataCacheConnector.fetchAtedRefData[String](Matchers.eq(AtedConstants.DelegatedClientAtedRefNumber))
      (Matchers.any(), Matchers.any(), Matchers.any())).thenReturn(Future.successful(Some("XN1200000100001")))
    when(mockService.saveDraftPropertyDetailsProfessionallyValued(Matchers.any(), Matchers.any())(Matchers.any(), Matchers.any())).
